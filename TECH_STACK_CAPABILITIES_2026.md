...
This project is a static, browser-first app (`index.html`, `styles.css`, `script.js`, `llm.js`, and `modules/*.js`) with no build step. The items below focus on **newer capabilities you can adopt incrementally** without changing the deployment model.
